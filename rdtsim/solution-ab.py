...
class EntityA:
   ...
   def output(self, message):
      if self.seqnum < self.seqnum_limit:
          pkt = Pkt(self.seqnum, self.acknum, 0, message.data)
          to_layer3(self, pkt)
          self.seqnum += 1

   def input(self, packet):  # 'packet' es una instancia de la clase Pkt
       if packet.acknum == self.seqnum:
           self.acknum = packet.acknum + 1
           self.window_size = 1

   def A_timerinterrupt(self, message):
      if self.seqnum < self.seqnum_limit:
          pkt = Pkt(self.seqnum, self.acknum, 0, message.data)
          to_layer3(self, pkt)
          self.seqnum += 1
