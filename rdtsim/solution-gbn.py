...
#Esta modificación verifica el valor del encabezado de la suma de comprobación del paquete y lo compara con el valor calculado. 
#Si no coinciden, el paquete se considera corrupto y se devuelve True. De lo contrario, se devuelve False.
...
def start_timer(calling_entity, increment):
    event = Event(self.time + increment, EventType.TIMER_INTERRUPT, calling_entity)
    self._insert_event(event)

def stop_timer(calling_entity):
    for i in range(len(self.event_list)):
        if (self.event_list[i].ev_type == EventType.TIMER_INTERRUPT
              and self.event_list[i].ev_entity == calling_entity):
...
class Simulator:
    ...
    #Esta modificación genera un tiempo aleatorio para la llegada del siguiente segmento de datos utilizando la función random.expovariate. 
    #Luego, inserta un evento de llegada en la lista de eventos. Este evento se procesará en la siguiente iteración del bucle principal.
    def _generate_next_arrival(self):
        if self.n_sim < self.n_sim_max - 1:
        # Generate the time for the next arrival
            time = self.time + random.expovariate(1/self.interarrival_time)

            # Insert the arrival event into the event list
            event = Event(time, EventType.FROM_LAYER5, None)
            self._insert_event(event)
        else:
            print('Simulation time limit exceeded.')
